          ...
          <div className="flex items-center gap-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className={cn("pl-3 text-left font-normal bg-white/80 hover:bg-white border-primary/20", isToday(selectedDate) && "bg-primary/10 border-primary/30")}>
                  <CalendarIcon className="mr-2 h-4 w-4 text-primary" />
                  <span className="font-medium">{getDateDisplay()}</span>
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar mode="single" selected={selectedDate} onSelect={date => date && setSelectedDate(date)} initialFocus className="p-3 pointer-events-auto" />
              </PopoverContent>
            </Popover>
            
            <Button variant="outline" size="sm" onClick={navigateNext} className="bg-white/80 hover:bg-white text-red-950">
              <ChevronRight className="h-4 w-4" />
            </Button>
            
            <Button variant="outline" size="sm" onClick={navigateToday} className={cn("bg-white/80 hover:bg-white", isToday(selectedDate) && "bg-primary/10 border-primary/30 font-medium")}>
              Today
            </Button>
          </div>
          
          <div className="flex items-center gap-2">
            <Tabs defaultValue="day" value={viewMode} onValueChange={value => setViewMode(value as ViewMode)} className="w-full sm:w-auto">
              <TabsList className="grid w-full grid-cols-2 bg-white/80">
                <TabsTrigger value="day" className="data-[state=active]:bg-primary/10 text-red-950">Day</TabsTrigger>
                <TabsTrigger value="week" className="data-[state=active]:bg-primary/10 text-red-950">Week</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </div>
