...
export const TimeGrid: React.FC<TimeGridProps> = ({ date, children, showTimeLabels = false }) => {
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {hourTimeSlots.map((slot, index) => (
        ...
        >
          {/* Time label - only show if showTimeLabels is true */}
          {showTimeLabels && (
      ...
      {isToday(date) && <CurrentTimeIndicator showTimeLabels={showTimeLabels} />}
    </div>
  );
};

const CurrentTimeIndicator: React.FC<{ showTimeLabels?: boolean }> = ({ showTimeLabels = false }) => {
  const { startHour, endHour } = useCalendarSettings();
  const now = new Date();
